...
(defn- policy-status-badge
  ...
  (if status
    [:wa-badge {:appearance "outlined"
                ...
                :pill       true}
     [:i18n/tr (insurance-ui/policy-status-label-key status)]]
    (ui2/muted nil)))
...
(defn- read-only-callout
  [{:keys [editable?] :as settings}]
  (when-not editable?
    (when-let [message-key (read-only-message-key settings)]
      [:wa-callout {:appearance "outlined" :variant "warning"}
       [:div {:class "wa-stack wa-gap-2xs"}
        [:strong [:i18n/tr :insurance/policy-settings-read-only-title]]
        [:span [:i18n/tr message-key]]]])))
      ...
      [:form {:id             "insurance-policy-settings-policy-form"
       ...
       [:div {:class "leading-none wa-grid" :style "--min-column-size: 18rem;"}
        ...
        (text-input {:id        "insurance-policy-settings-effective-at"
                     :label     [:i18n/tr :insurance/effective-at]
                     :type      "date"
        ...
        (text-input {:id        "insurance-policy-settings-effective-until"
                     :label     [:i18n/tr :insurance/effective-until]
                     :type      "date"
        ...
        (text-input {:id        "insurance-policy-settings-premium-factor"
                     :label     [:i18n/tr :insurance/premium-base-factor]
                     :type      "number"
        ...
        (currency-select {:id                   "insurance-policy-settings-currency"
                          :label                [:i18n/tr :insurance/currency]
                          :selected             currency
        ...
        [:div {:class "wa-stack wa-gap-2xs"}
         [:span {:class "wa-caption-s wa-font-weight-bold"}
          [:i18n/tr :insurance/dashboard-policy-status]]
         [:div {:class "wa-cluster wa-gap-xs"}
          (policy-status-badge status)
          [:span {:class "wa-caption-s wa-color-text-quiet"}
           [:i18n/tr :insurance/policy-settings-status-read-only]]]]
...
(defn- exporter-version-option
  [selected-exporter-id {:keys [exporter-id label-key]}]
  ...
  (let [known-exporter-ids (set (map :exporter-id exporter-options))]
    (native-field
     {:error (field-error _error :exporter-id)
      ...
      :label [:i18n/tr :insurance/exporter]}
     (into
      [:select
...
(defn- exporter-role-select
  [coverage-type-rows exporter-id idx
     ...
     (into
      [:select (cond-> {:id                 id
       ...
       [:option (cond-> {:value ""}
                  ...
                  (assoc :selected true))
        [:i18n/tr :insurance/exporter-role-unmapped]]]
  ...
  (let [{:keys [_error exporter-id mappings policy-id] :as form}
    ...
    (settings-card
     ...
     [:form (cond-> {:id             "insurance-policy-settings-exporter-form"
      ...
      (top-error-callout (:_top _error))
      (exporter-version-select settings form disabled?)
...
(defn- current-totals-section
  [{{:keys [currency]} :policy-details :keys [current-totals]}]
  (settings-card
   {:title    [:i18n/tr :insurance/policy-settings-current-totals]
    :subtitle [:i18n/tr :insurance/policy-settings-current-totals-subtitle]}
   [:dl {:class "leading-none wa-grid" :style "--min-column-size: 12rem;"}
    (metric-item [:i18n/tr :insurance/dashboard-total-instruments]
                 (:total-instruments current-totals 0))
    (metric-item [:i18n/tr :insurance/dashboard-total-insured-value]
                 (ui2/money (:total-insured-value current-totals 0M) currency))
    (metric-item [:i18n/tr :insurance/dashboard-policy-cost]
                 (ui2/money (:total-cost current-totals 0M) currency))]))

(defn- warning-callout
  [{:keys [category-names type]}]
  (case type
    :missing-category-factors
    [:wa-callout {:appearance "outlined" :variant "warning"}
     [:div {:class "wa-stack wa-gap-2xs"}
      [:strong [:i18n/tr :insurance/policy-settings-missing-category-factors-title]]
      [:span [:i18n/tr :insurance/policy-settings-missing-category-factors-body
              {:category-names (str/join ", " category-names)}]]]]
    nil))
...
(defn- coverage-type-icon-field
  [icon error id-prefix]
  (into
   [:wa-combobox
    (cond-> {:id         (str id-prefix "-icon")
             :label      [:i18n/tr :insurance/coverage-type-icon]
             :hint       [:i18n/tr :insurance/coverage-type-icon-hint]
...
(defn- coverage-type-impact-confirmation
  [{:keys [_error confirmation-count impact-count]} id-prefix]
  (when (and (number? impact-count) (pos? impact-count))
    [:div {:class "wa-stack wa-gap-xs"}
     [:span
      [:i18n/tr :insurance/coverage-type-impact-confirmation
       {:count impact-count}]]
     (text-input
      {:id       (str id-prefix "-confirmation-count")
       :label    [:i18n/tr :insurance/coverage-type-confirmation-count
                  {:count impact-count}]
       :hint     [:i18n/tr :insurance/coverage-type-confirmation-count-hint]
       :type     "number"
...
(defn- coverage-type-fields
  ...
  [:div {:class "wa-stack wa-gap-m"}
   ...
   [:wa-checkbox
    ...
    [:i18n/tr :insurance/coverage-type-required]]
   (when create?
     [:wa-checkbox
      (cond-> {:id                (str id-prefix "-add-to-band-instruments")
               :hint              [:i18n/tr :insurance/coverage-type-add-to-band-instruments-hint]
      ...
      [:i18n/tr :insurance/coverage-type-add-to-band-instruments]])
   (coverage-type-impact-confirmation form id-prefix)
   (text-input {:id    (str id-prefix "-premium-factor")
                :label [:i18n/tr :insurance/premium-factor]
                :type  "number"
                :value (str premium-factor)
  ...
  (let [{:keys [open policy-id] :as form} (coverage-type-create-state req settings)]
    (when open
      [:wa-dialog {:id                    "coverage-type-create-dialog"
                   :label                 [:i18n/tr :insurance/policy-settings-add-coverage-type]
       ...
       [:form {:id             "coverage-type-create-form"
        ...
        (coverage-type-fields form {:create?   true
  ...
  (let [{:keys [policy-id type-id] :as form} (coverage-type-edit-state req)]
    (when type-id
      [:wa-dialog {:id                    "coverage-type-edit-dialog"
                   :label                 [:i18n/tr :insurance/policy-settings-edit-coverage-type]
                   :data-init__delay.10ms "el.open = true"
                   :data-preserve-attr    "open"
       ...
       [:form {:id             "coverage-type-edit-form"
        ...
        (coverage-type-fields form {:id-prefix "coverage-type-edit"})]
       [button/Button {:slot        "footer"
                       :appearance  "outlined"
                       :data-dialog "close"}
        [:i18n/tr :action/cancel]]
...
(defn- coverage-type-delete-dialog
  [req {:keys [name type-id usage-count used?]}]
  (let [dialog-id  (ui2/remove-dialog-id "coverage-type" type-id)
    ...
    [:wa-dialog {:id    dialog-id
     ...
     [:div {:class "wa-stack wa-gap-s"}
      [:p [:i18n/tr :insurance/policy-settings-coverage-type-delete-confirm
           {:coverage-type-name name}]]
      (when used?
        [:wa-callout {:appearance "outlined" :variant "warning"}
         [:div {:class "wa-stack wa-gap-2xs"}
          [:strong [:i18n/tr :insurance/policy-settings-error-coverage-type-in-use]]
          [:span [:i18n/tr :insurance/policy-settings-coverage-type-in-use
                  {:count usage-count}]]]])]
     [button/Button {:slot        "footer"
                     :appearance  "outlined"
                     :data-dialog "close"}
...
(defn- coverage-type-row
  [{:keys [editable?] :as req} currency {:keys [current-cost description name premium-factor type-id usage-count used?]}]
             ...
             (ui2/row-action-menu
              {:button-id (str "coverage-type-actions-" type-id)
               :items     [{:label              [:i18n/tr :action/update]
                           ...
                           (cond-> {:label       [:i18n/tr :action/remove]
                                    :variant     "danger"
...
(defn- coverage-type-table-head
  [editable?]
  (table-head (cond-> [[:i18n/tr :insurance/coverage-name]
                       [:i18n/tr :insurance/premium-factor]
                       [:i18n/tr :insurance/coverage-type-description]
                       [:i18n/tr :insurance/policy-settings-usage]
                       [:i18n/tr :insurance/policy-settings-current-cost]]
                editable? (conj [:i18n/tr :actions]))))
...
(defn- category-factor-create-fields
  [{:keys [_error category-id factor]} unused-categories id-prefix]
  [:div {:class "wa-stack wa-gap-m"}
   (top-error-callout (:_top _error))
   (category-select {:id         (str id-prefix "-category")
                     :label      [:i18n/tr :instrument/category]
...
(defn- category-factor-edit-fields
  [{:keys [_error category-id category-name factor]} id-prefix]
  [:div {:class "wa-stack wa-gap-m"}
   (top-error-callout (:_top _error))
   [:input {:type      "hidden"
            :value     category-id
            :data-bind "insurancePolicySettings.categoryFactor.categoryId"}]
   (read-only-field [:i18n/tr :instrument/category] category-name)
   (text-input {:id    (str id-prefix "-factor")
                :label [:i18n/tr :insurance/premium-factor]
                :type  "number"
                :value (str factor)
    ...
    (when open
      [:wa-dialog {:id                    "category-factor-create-dialog"
       ...
       [:form {:id             "category-factor-create-form"
        ...
        (category-factor-create-fields form unused-categories "category-factor-create")]
       [button/Button {:slot        "footer"
                       :appearance  "outlined"
                       :data-dialog "close"}
        [:i18n/tr :action/cancel]]
  ...
  (let [{:keys [category-factor-id policy-id] :as form} (category-factor-edit-state req)]
    (when category-factor-id
      [:wa-dialog {:id                    "category-factor-edit-dialog"
                   :label                 [:i18n/tr :insurance/policy-settings-edit-category-factor]
                   :data-init__delay.10ms "el.open = true"
                   :data-preserve-attr    "open"
       ...
       [:form {:id             "category-factor-edit-form"
        ...
        (category-factor-edit-fields form "category-factor-edit")]
       [button/Button {:slot        "footer"
                       :appearance  "outlined"
                       :data-dialog "close"}
        [:i18n/tr :action/cancel]]
...
(defn- category-factor-delete-dialog
  [req {:keys [category-factor-id category-name usage-count used?]}]
  (let [dialog-id  (ui2/remove-dialog-id "category-factor" category-factor-id)
        loading-id (pr-str (str category-factor-id))]
    [:wa-dialog {:id    dialog-id
                 :label [:i18n/tr :action/confirm-generic]}
     [:div {:class "wa-stack wa-gap-s"}
      [:p [:i18n/tr :insurance/policy-settings-category-factor-delete-confirm
           {:category-name category-name}]]
      (when used?
        [:wa-callout {:appearance "outlined" :variant "warning"}
         [:div {:class "wa-stack wa-gap-2xs"}
          [:strong [:i18n/tr :insurance/policy-settings-error-category-factor-in-use]]
          [:span [:i18n/tr :insurance/policy-settings-category-factor-in-use
                  {:count usage-count}]]]])]
     [button/Button {:slot        "footer"
                     :appearance  "outlined"
                     :data-dialog "close"}
...
(defn- category-factor-row
  [{:keys [editable?] :as req} currency {:keys [category-factor-id category-name current-cost factor usage-count used?]}]
...
(defn- category-factor-table-head
  [editable?]
  (table-head (cond-> [[:i18n/tr :instrument/category]
                       [:i18n/tr :insurance/premium-factor]
                       [:i18n/tr :insurance/policy-settings-usage]
                       [:i18n/tr :insurance/policy-settings-current-cost]]
                editable? (conj [:i18n/tr :actions]))))
...
(defn- category-factor-create-actions
  [req policy-id unused-categories]
  (if (seq unused-categories)
    [[button/Button {:appearance  "outlined"
                     :variant     "brand"
                     :data-id     policy-id
                     :data-action (d*/act req ::actions/open-category-factor-create)}
      [:i18n/tr :insurance/policy-settings-add-category-factor]]]
    [[:span {:id    category-factor-create-disabled-id
     ...
     [:wa-tooltip {:for category-factor-create-disabled-id}
      [:i18n/tr :insurance/policy-settings-category-factor-create-disabled-tooltip]]]))
   ...
   [:div {:class "wa-stack wa-gap-m"}
    ...
    (ui2/table-shell
     (if (seq category-factor-rows)
       [:table
        (category-factor-table-head editable?)
        (into [:tbody]
              (map (partial category-factor-row (assoc req :editable? editable?) currency))
              category-factor-rows)]
       (ui2/empty-state [:i18n/tr :insurance/policy-settings-no-category-factors] "")))]))
...
(defn settings-page-content
  ...
  [:div {:id           "insurance-policy-settings"
   ...
   [page-header/PageHeader
    {:title    [:i18n/tr :insurance/policy-settings]
     :subtitle [:i18n/tr :insurance/policy-settings-subtitle
   ...
   [:div {:class "wa-flank:end wa-align-items-start" :style "--flank-size: 34ch;"}
    ...
    [:aside {:class "wa-stack"}
     (current-totals-section settings)]]
