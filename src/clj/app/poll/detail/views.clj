...
(defn- open-dialog [req poll]
  (let [id      (dialog-id "poll-open" poll)
        poll-id (str (:poll/poll-id poll))]
    [:wa-dialog {:id                 id
                 :label              [:i18n/tr :polls/open-poll]
                 :data-preserve-attr "open"}
     [:p [:i18n/tr :polls/open-hint]]
     ...
     [button/Button (merge {:slot        "footer"
      ...
      [:i18n/tr :polls/confirm-open-poll]]]))

(defn- close-dialog [req poll]
  (let [id      (dialog-id "poll-close" poll)
        poll-id (str (:poll/poll-id poll))]
    [:wa-dialog {:id                 id
                 :label              [:i18n/tr :polls/close-early]
                 :data-preserve-attr "open"}
     [:p [:i18n/tr :polls/confirm-close-poll {:title (:poll/title poll)}]]
     [button/Button {:slot        "footer"
                     :appearance  "outlined"
                     :data-dialog "close"}
      [:i18n/tr :action/cancel]]
