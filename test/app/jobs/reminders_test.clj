    ...
    (fixtures/with-runtime
      (fn [runtime client conn]
        (let [gig-id       (random-uuid)
              ...
              system       {:frame-loop (when durable? runtime)                                                       :datomic {:conn conn} :job-queue {:client client}
          ...
          (let [db (d/db conn)
            ...
            (is (= (repeat 3 (if durable? :reminder-status/queued :reminder-status/sent))
                   (mapv #(:reminder/reminder-status (d/entity db [:reminder/reminder-id %])) reminder-ids)))
            (when durable? (is (string? (:audit/jobs tx)))))
