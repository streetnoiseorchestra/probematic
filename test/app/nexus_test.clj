...
(defn queued-events [config conn actions]
  (let [tab-id  (str (random-uuid))
        token   (Object.)
        runtime {:clients (atom {tab-id {:token token :revision 0 :events []}})}
        request {:body-params {:tab-id tab-id}             ::datastar/state-token token
                 :tr          (constantly "Action failed")}]
    (app-nexus/process-queued! config {:datomic {:conn conn}} runtime
                               {:request request :actions actions})
    (mapcat second (get-in @(:clients runtime) [tab-id :events]))))

(deftest queued-actions-preserve-ordered-events-and-nested-transaction-feedback
  (let [{:keys [conn]} (tc/new-system "queued-feedback")
        team-id        (random-uuid)
        events         [[:app.datastar.sse/merge-signals {:saved true}]
                        [:app.datastar.sse/redirect "/done"]]]
    (is (= events (queued-events (app-nexus/nexus) conn [[:app.datastar/respond-sse events]])))
    (is (= events (queued-events (app-nexus/nexus) conn
                                 [[:db/transact [{:team/team-id team-id :team/name "Saved"}]
                                   {:on-success [[:app.datastar/respond-sse events]]}]])))
    (is (= "Saved" (:team/name (d/entity (d/db conn) [:team/team-id team-id]))))))

(deftest queued-actions-reject-competing-response-owners
  (let [{:keys [conn]} (tc/new-system "queued-conflict")
        events         [[:app.datastar.sse/redirect "/must-not-redirect"]]
        team-id        (random-uuid)]
    (doseq [actions [[[:app.datastar/respond-sse events]
                      [:app.datastar/respond-sse events]]
                     [[:db/transact [{:team/team-id team-id :team/name "Committed"}]
                       {:on-success [[:app.datastar/respond-sse events]]}]
                      [:app.datastar/respond-sse events]]]]
      (is (= [:app.datastar.sse/merge-signals :app.datastar.sse/execute-script]
             (mapv first (queued-events (app-nexus/nexus) conn actions)))))
    (is (= "Committed" (:team/name (d/entity (d/db conn) [:team/team-id team-id]))))))

(deftest failed-queued-effects-do-not-emit-success
  (let [{:keys [conn]} (tc/new-system "queued-failure")
        after          (atom false)
        config         (-> (app-nexus/nexus)
                           (assoc-in [:nexus/effects ::fail] (fn [_ _] (throw (ex-info "Expected failure" {}))))
                           (assoc-in [:nexus/effects ::after] (fn [_ _] (reset! after true))))]
    (is (= [:app.datastar.sse/merge-signals :app.datastar.sse/execute-script]
           (mapv first (queued-events config conn
                                      [[::fail] [::after]
                                       [:app.datastar/respond-sse [[:app.datastar.sse/redirect "/success"]]]]))))
    (is (false? @after))))
