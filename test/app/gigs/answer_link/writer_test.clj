...
(deftest answer-links-read-current-writer-state-and-trigger-follow-up-outside-it
  (fixtures/with-runtime
    (fn [runtime _ conn]
      (let [{:keys [gig-id member-id]} (writer/call! (:write-runner runtime)
            ...
            req                        (assoc-in (answers/req conn {:gig/gig-id gig-id :member/member-id member-id :attendance/plan :plan/definitely})
                                                 [:system :frame-loop] runtime)
            entered                    (promise)
            release                    (promise)
            follow-up-thread           (promise)]
        ...
        (try
          ...
          (let [result (future (service/submit-answer!
                                (assoc answers/deps :trigger-gig-edited! (fn [& _] (deliver follow-up-thread (Thread/currentThread))))
                                req))]
            ...
            (is (not (identical? (::game/thread runtime) (deref follow-up-thread 5000 nil)))))
